...
from document_inteligence.domain.repositories import (
    CaptionVerifier,
    DocumentIngestor,
...
from document_inteligence.application.rag_pack import RagChunk, build_rag_chunks
...
def _build_reading_order_strategy(config: ParserConfig) -> ReadingOrderStrategy:
    ...
    if name == "xy_cut":
        from document_inteligence.infrastructure.strategies.xy_cut_strategy import (
            XYCutStrategy,
        )
        return XYCutStrategy(gap_ratio=config.xy_cut_gap_ratio)
    from document_inteligence.infrastructure.strategies.composite_strategy import (
        CompositeStrategy,
    )
    from document_inteligence.infrastructure.strategies.xy_cut_strategy import (
        XYCutStrategy,
